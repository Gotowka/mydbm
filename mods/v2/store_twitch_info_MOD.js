module.exports = {
  ...
  meta: {
    ...
    downloadURL: 'https://github.com/dbm-network/mods/blob/master/actions/store_twitch_info_MOD.js',
  },

  subtitle(data) {
    const sourceType = parseInt(data.type, 10); // "Channel", "Stream", "Video" or "Game"
    const inputType = parseInt(data.inputtype, 10); // "ID" or "Name"
...
<style>
  ...
  async action(cache) {
    ...
    const inputType = parseInt(data.inputtype, 10); // Channel "ID" or "Name"
    const type = parseInt(data.type, 10); // "Channel", "Stream", "Video" or "Game"
    let infoType;
    switch (type) {
      case 0:
        infoType = parseInt(data.info1, 10);
        break;
      case 1:
        infoType = parseInt(data.info2, 10);
        break;
      case 2:
        infoType = parseInt(data.info3, 10);
        break;
      case 3:
        infoType = parseInt(data.info4, 10);
        break;
      default:
        break;
    }
    ...
    switch (type) {
      ...
      case 2: // Video
        ...
        if (json && json.data.length !== 0) {
          ...
          switch (infoType) {
            case 0:
              result = json.data.map((video) => video.id);
              break;
            case 1:
              result = json.data[0].user_id;
              break;
