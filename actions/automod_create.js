module.exports = {
  ...
  meta: { version: "3.2.4", preciseCheck: true, author: 'Gotowka', authorUrl: 'https://github.com/Gotowka', downloadUrl: 'https://github.com/Gotowka/mydbm/blob/v3/actions/automod_create.js' },
  ...
  async action(cache) {
    console.log('\x1b[30m[\x1b[35mACTION\x1b[30m]: \x1b[33mautomod_create; \x1b[30m[\x1b[32mv1.0\x1b[30m] \x1b[30m(\x1b[36mv3.2.4\x1b[30m)\x1b[0m')
    ...
    const settings = {
      ...
      eventType: 1,
      triggerType: option.number,
      enabled: true,
      actions: [],
      triggerMetadata: {}
    }

    if (autoMod.delete) settings.actions.push({ type: 1 });
    if (autoMod.alert) {
      ...
      if (autoMod.autoMsg) settings.actions.push({ type: 2, metadata: { channel: targetChannel, customMessage: this.evalMessage(autoMod.autoMsg, cache) } });
      else settings.actions.push({ type: 2, metadata: { channel: targetChannel } });
    };
    if (autoMod.timeout) settings.actions.push({ type: 3, metadata: { durationSeconds: autoMod.autoTime ? this.evalMessage(autoMod.autoTime, cache) : 10 } });
    ...
    function getAutomod() {
      if (data.aspam) return { number: 3, word: "spam" };
      else if (data.akeyword) return { number: 1, word: 'keyword' };
      else if (data.amentionspam) return { number: 5, word: 'mention_spam' };
      else return "none";
    }
  },
