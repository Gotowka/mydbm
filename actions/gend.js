module.exports = {
    ...
    meta: { version: "3.2.1", preciseCheck: true, author: 'Gotowka', authorUrl: 'https://github.com/Gotowka', downloadUrl: 'https://github.com/Gotowka/mydbm/blob/v3/actions/gend.js' },
...
<input id="label" class="round" placeholder="[users] = Users from the giveaway" type="text">
  ...
  `;
    ...
    async action(cache) {
      console.log('\x1b[30m[\x1b[35mACTION\x1b[30m]: \x1b[33mgend; \x1b[30m[\x1b[32mv1.1\x1b[30m] \x1b[30m(\x1b[36mv3.2.1\x1b[30m)\x1b[0m')
            ...
            for (var i in giveaways[giveawayid]) {
                if (new Date().getTime() > giveaways[giveawayid][i].end && giveaways[giveawayid][i].ended == false) {
                    ...
                    const button = new ActionRowBuilder().addComponents(
                        new ButtonBuilder()
                        .setCustomId('gend')
                        .setStyle((this.evalMessage(data.style, cache)).replace('DANGER', 4).replace('PRIMARY', 1).replace('SECONDARY', 2).replace('SUCCESS', 3))
                        .setDisabled(true)
